...
public abstract class MultiObjectiveConstraintBasedAllocAlgorithm extends ConstraintBasedAllocAlgorithm {
    ...
    public MultiObjectiveConstraintBasedAllocAlgorithm(VMCwMProblem instance, boolean break_symms) {
        ...
        this.wastage_coeffs_cache =
                ANNOTATE_SOLVER_STRUCTS ? new @Gen HashMap<Pair<BigInteger,BigInteger>, Pair<IVec<BigDecimal>,IVec<BigDecimal> > >()
    ...
    private IVec<BigDecimal> indexWiseSubtraction(double[] a1, double[] a2) {
        ...
        IVec<BigDecimal> sub_array = ANNOTATE_SOLVER_STRUCTS ? new @Gen Vec<BigDecimal>(a1.length) : new Vec<BigDecimal>(a1.length);
        for (int i = 0; i < a1.length; ++i) {
            sub_array.push(ANNOTATE_SOLVER_STRUCTS ? new @Gen BigDecimal(a1[i] - a2[i]) : new BigDecimal(a1[i] - a2[i]));
        }
        return sub_array;
    }
    ...
    protected ConstraintSolver buildSolver() throws ContradictionException {
        ...
        ConstraintSolver solver = ANNOTATE_SOLVER_STRUCTS ? new @Gen PseudoBooleanSolver() : new PseudoBooleanSolver();
    ...
    private void initializeEnergyObjectiveFunction() {
        ...
        this.energy_lits = ANNOTATE_SOLVER_STRUCTS ? new @Gen VecInt() : new VecInt();
        this.energy_coeffs = ANNOTATE_SOLVER_STRUCTS ? new @Gen Vec<BigDecimal>() : new Vec<BigDecimal>();
        for (int i = 0; i < this.instance.getPhysicalMachines().size(); ++i) {
            PhysicalMachine pm = this.instance.getPhysicalMachines().get(i);
            this.energy_coeffs.push(ANNOTATE_SOLVER_STRUCTS ? new @Gen BigDecimal(pm.getIdleConsumption()) : new BigDecimal(pm.getIdleConsumption()));
            this.energy_lits.push(this.pm_vars.get(i));
            int energy_range = pm.getMaxConsumption() - pm.getIdleConsumption();
            double[] norm_cpus = getNormalizedCPURequirements(vms, pm);
            for (int j = 0; j < vms.size(); ++j) {
                this.energy_coeffs.push(ANNOTATE_SOLVER_STRUCTS ? new @Gen BigDecimal(energy_range * norm_cpus[j]) : new BigDecimal(energy_range * norm_cpus[j]));
    ...
    private void initializeWastageObjectiveFunction() {
        ...
        this.wastage_lits = ANNOTATE_SOLVER_STRUCTS ? new @Gen VecInt() : new VecInt();
        this.wastage_coeffs = ANNOTATE_SOLVER_STRUCTS ? new @Gen Vec<BigDecimal>() : new Vec<BigDecimal>();
        for (int i = 0; i < this.instance.getPhysicalMachines().size(); ++i) {
            PhysicalMachine pm = this.instance.getPhysicalMachines().get(i);
            Pair<IVec<BigDecimal>,IVec<BigDecimal> > coeff_vec_pair = getWastageCoefficients(pm, vms);
            IVec<BigDecimal> norm_mem_minus_cpu = coeff_vec_pair.getLeft();
    ...
    private void initializeMigrationObjectiveFunction() {
        ...
        this.migration_lits = ANNOTATE_SOLVER_STRUCTS ? new @Gen VecInt() : new VecInt();
        this.migration_coeffs = ANNOTATE_SOLVER_STRUCTS ? new @Gen Vec<BigInteger>() : new Vec<BigInteger>();
    ...
    protected IVecInt buildUndefFormulas() {
        IVecInt undef_fmls = ANNOTATE_CONSTRAINTS_EXTERNAL ? new @Gen VecInt() : new VecInt();
